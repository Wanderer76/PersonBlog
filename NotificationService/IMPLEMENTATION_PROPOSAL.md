...
Дата: 5 сентября 2026 года. Статус: предложение, частично реализовано.
...
Актуализация от 10 сентября 2026 года: реализованы durable-контур Notification, события публикации поста, результата обработки видео, ответа на комментарий и приглашения в конференцию, transactional outbox у producers, fanout по подписчикам, REST API истории и настроек, Gateway-клиент и запуск Notification API в AppHost. Не реализованы frontend и Compose/Nginx-маршруты.
...
| Создание текстового поста | `BlogService/Blog.Service/Services/Implementation/DefaultBlogPostV2Service.cs`, `CreatePostAsync` | Создаётся `PostCatalogChangedV2`, но не `PostUpdateEvent(Create)`. Текущий Notification consumer этот сценарий пропускает |
...
1. **Неверная граница сервиса подписок в старой заготовке.** Обработчик обращался к несуществующему `api/InternalPost/blogSubscribers/...`. В реализованном варианте Blog предоставляет явный `api/internal/blogs/{blogId}/subscribers`, соответствующий находящейся в Blog проекции `Subscriber`.
...
NotificationService/
  ...
  Notification.Infrastructure/
    ...
    Clients/                   BlogRecipientDirectory
...
### MVP: чтение страниц из Blog

Добавить внутренний endpoint в **BlogService**, например `GET /api/internal/blogs/{blogId}/subscribers?cursor=...&limit=500&cutoff=...`, защищённый сервисной аутентификацией. Контракт ответа: `userIds`, `nextCursor`, `hasMore`. Blog владеет используемой в текущей архитектуре проекцией `Subscriber`.

Использовать keyset pagination по неизменяемой паре `(SubscriptionStartDate, UserId)` и соответствующий индекс `(BlogId, SubscriptionStartDate, UserId)`. Cursor формирует Blog; Notification хранит и передаёт его непрозрачно. Offset/номер страницы при конкурентной отписке может пропустить пользователей. Повтор запроса одной страницы должен быть безопасен, а порядок сортировки — явным.
...
**Семантика аудитории MVP:** пользователи с активной подпиской на момент чтения их страницы, созданной не позже PublishedAt. Это не строгий снимок «все подписчики в момент публикации»: Blog физически удаляет подписку, а отписка/повторная подписка во время обхода меняет состав. Ограничение явно принять для MVP; cutoff не решает его полностью. Повторная проверка перед внешней отправкой уменьшает число уведомлений после отписки, но гонку с самой отправкой полностью не устраняет.

Если нужен точный снимок, Blog должен хранить историю интервалов подписки либо материализовать аудиторию кампании у себя и выдавать стабильные страницы по SnapshotId. Нельзя обещать точную историческую аудиторию по текущей таблице активных подписок.

### При росте нагрузки: локальная проекция

Notification может поддерживать собственную read-модель подписок через события владельца подписок. Но текущего `SubscriptionChangedV1` недостаточно для надёжного порядка unsubscribe/resubscribe: нужны монотонная версия отношения и tombstone последней отписки. Также нужны начальный snapshot с watermark, воспроизведение изменений после watermark и процедура сверки. Даже такая проекция не гарантирует точную аудиторию на момент события из другого сервиса без согласованного правила времени/водяного знака. Поэтому для начала внутренний API проще и честнее по гарантиям.
...
Минимальные метрики: возраст старейшего outbox/inbox/job, длина очереди, число created/deduplicated/failed, latency от OccurredAt до сохранения, длительность и прогресс кампании, HTTP-ошибки Blog recipient API, ошибки SignalR/провайдеров. Раздельно измерять принятие события, создание истории и попытку внешней доставки.

Liveness проверяет живость процесса, readiness — зависимости соответствующей роли. Падение Redis не должно лишать REST доступа к истории; падение Blog recipient API задерживает только кампании подписчиков. Настроить алерты на растущий backlog и failed jobs, административный просмотр и replay карантина с авторизацией и аудитом.

Notification.API, Domain и Contract уже включены в solution; добавить туда новые Application/Infrastructure и тестовые проекты. Подключить Notification API к AppHost и Compose; добавить Dockerfile, конфигурации PostgreSQL/RabbitMQ/Blog/Auth, Nginx hub route, Gateway client и Swagger. Передать секреты штатным способом окружения. Отдельно проверить URL BaseAddress и `/api/`, учитывая исторические имена Profile/Reacting. Миграции применять управляемо до запуска новых consumers.
...
| 4. Публикации | Единое правило публикации для текста/видео, PublicationId, Blog recipient API, FanoutCampaign | Только первая публичная публикация запускает рассылку; restart посередине пачки безопасен |
