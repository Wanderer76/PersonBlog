...
Актуализация от 7 сентября 2026 года: реализованы durable-контур Notification, событие `PostPublishedV1`, fanout по подписчикам, REST API истории и настроек, Gateway-клиент и запуск Notification API в AppHost. Владельцем внутреннего API аудитории выбран Blog, поскольку актуальная проекция `Subscriber` находится в его границе. Не реализованы frontend, Compose/Nginx-маршруты и события Comments/Conference.
