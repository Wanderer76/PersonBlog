...
public class PostBanService {
    ...
    public PostBanService(RabbitTemplate rabbitTemplate, PostBanRequestRepository postBanRequestRepository) {
    ...
    public PostBanRequestsViewModel getBanRequestList(int page, int size) {
        ...
        return new PostBanRequestsViewModel(count, data.stream().map(x ->
                        new PostBanRequestItemModel(x.getId(), x.getPostId(), x.getFilename(), x.getReasonId(), x.getUserMessage(), x.getCreatedAt()))
                .toList())
                ;
    }
