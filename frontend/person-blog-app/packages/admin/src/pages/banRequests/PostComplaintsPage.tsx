...
import type { PostBanRequest } from "../../types/BanTypes";
import styles from "./BanRequests.module.css";
import { useNavigate } from "react-router-dom";
import BanRequestsPage from "./BanRequestsPage";

// Новые типы для модели
interface PostToBanItem {
  postId: string;
  requestCount: number;
  postTitle: string;
}

interface BanPostViewModel {
  count: number;
  items: PostToBanItem[];
}
...
export default function PostComplaintsPage() {
  ...
  const [banRequests, setBanRequests] = useState<PostToBanItem[]>([]);
  ...
  async function fetchBanRequests(page = 0, size = 10) {
    const url = new URL(`${BaseApUrl}/api/post/postToBan/list`);
    ...
    const data = (await res.data) as BanPostViewModel;
  ...
  // Функция для открытия деталей поста
  ...
  if (selectedPostId) {
    return <BanRequestsPage postId={selectedPostId} onBack={handleBackToList} />;
  }
        ...
        <div className="col-12">
          ...
          {!loading && totalCount === 0 ? (
            ...
            banRequests.map((request) => (
              <div key={request.postId} className={styles.requestCard}>
                <div className={styles.requestHeader}>
                  <div className={styles.requestHeaderContent}>
                    ...
                    <span className={`badge bg-primary ${styles.badge}`}>
                      <i className={`fas fa-exclamation-circle ${styles.clockIcon}`}></i>
                      {request.requestCount} жалоб
                    </span>
                  </div>
                </div>
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      ...
                      >
                        ...
                        <code className={styles.code}>{request.postTitle}</code>
                      </div>

                    </div>
                    <div className="col-md-6">
                      <div className="mb-2">
                        <strong>Количество жалоб:</strong>
          ...
          {totalPages > 1 && (
            <div className={styles.paginationContainer}>
              ...
              <div className={styles.pageInfo}>
                Показано {banRequests.length} из {totalCount} постов с жалобами
              </div>
            </div>
          )}
        </div>
